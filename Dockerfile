...
# usage:
#
# docker run -v `pwd`:/files -it navicore/acs-engine oe-k8s.json
#
# helpful aliases:
#
#
...
RUN mkdir -p /gopath/src/github.com/Azure && cd /gopath/src/github.com/Azure && git clone https://github.com/Azure/acs-engine.git
RUN cd /gopath/src/github.com/Azure/acs-engine/ && make ci

WORKDIR /files
ENTRYPOINT ["/gopath/bin/acs-engine"]
